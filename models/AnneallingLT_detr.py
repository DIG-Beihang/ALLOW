...
class SetCriterion(nn.Module):
    ...
    def __init__(self, args, num_classes, matcher, weight_dict, losses, invalid_cls_logits, focal_alpha=0.25):
        ...
        self.cooling_prev = args.cooling_prev
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        out_logits = torch.gather(out_logits, 1, topk_boxes.unsqueeze(-1).repeat(1,1,81))

        results = [{'scores': s, 'labels': l, 'boxes': b, 'logits': o,} for s, l, b, o in zip(scores, labels, boxes, out_logits)]
