...
def get_args_parser():
    ...
    parser.add_argument('--cooling', default=False, action='store_true')
    parser.add_argument('--cooling_prev', default=0, type=int)
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.dataset in ['owod'] and (epoch % args.eval_every == 0 and epoch > 0 or epoch == args.epochs - 1):
            test_stats, coco_evaluator = evaluate(
                model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, args
            )
        else:
