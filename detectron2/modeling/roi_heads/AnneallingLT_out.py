...
class FastRCNNOutputs:
    ...
    def __init__(
        ...
        if len(proposals):
            ...
            if proposals[0].has("gt_boxes"):
                ...
                assert proposals[0].has("ori_classes")
                self.ori_classes = cat([p.ori_classes for p in proposals], dim=0)
        else:
            self.proposals = Boxes(torch.zeros(0, 4, device=self.pred_proposal_deltas.device))
        self._no_instances = len(proposals) == 0  # no instances found
        self.weights = torch.ones(self.pred_class_logits.size(1)).cuda()
        self.valid_class_range = [i for i in range(80) if i not in invalid_class_range]
        self.weights[self.valid_class_range] = weight_known
        self.cooling = cooling
        self.peak = peak
        self.cool_iter = cool_iter
        self.beta = beta
    ...
    def one_hot_cross_loss(self):
        if self._no_instances:
            ...
            num_classes = self.pred_class_logits.size(1)
            num_batch = self.gt_classes.size(0)
            # import pdb; pdb.set_trace()
            device = self.pred_class_logits.device
            gt_labels = torch.full(size=(num_batch, num_classes), fill_value=0, device = device)
            gt_labels.scatter_(dim=1, index=torch.unsqueeze(self.gt_classes, dim=1), value=1)

            ori_labels = torch.full(size=(num_batch, num_classes), fill_value=0, device = device)
            ori_labels.scatter_(dim=1, index=torch.unsqueeze(self.ori_classes, dim=1), value=1)
            
            labels = gt_labels | ori_labels
            
            # import pdb; pdb.set_trace()
            log_prob = F.log_softmax(self.pred_class_logits, dim=1)
            loss = -torch.sum(log_prob * labels * self.weights) / torch.sum(torch.sum(labels, dim=0) * self.weights)
            return loss

    def del_tensor_col(self, arr, index):
        arr1 = arr[:,0:index]
        arr2 = arr[:,index+1:]
        return torch.cat((arr1,arr2),dim=1)

    def two_cross_loss(self):
        ...
        if self._no_instances:
            ...
            ori_labels = torch.full(size=(num_batch, num_classes), fill_value=0, device = device)
            ori_labels.scatter_(dim=1, index=torch.unsqueeze(self.gt_classes, dim=1), value=1)
            
            labels = gt_labels | ori_labels

            log_prob_1 = F.log_softmax(self.pred_class_logits, dim=1)
            loss_1 = -torch.sum(log_prob_1 * labels) / num_batch
            
            # import pdb; pdb.set_trace() 
            ori_labels_del_unk =  self.del_tensor_col(ori_labels, 80)
            pred_del_unk = self.del_tensor_col(self.pred_class_logits, 80)

            log_prob_del_unk = F.log_softmax(pred_del_unk, dim=1)
            loss_del_unk = -torch.sum(log_prob_del_unk * ori_labels_del_unk) / num_batch
        
            cross_loss = loss_1 + loss_del_unk
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.cls_score = Linear(self.input_size, num_classes + 1)
        self.num_bbox_reg_classes = 1 if cls_agnostic_bbox_reg else num_classes
        self.box_dim = len(box2box_transform.weights)
        self.bbox_pred = Linear(self.input_size, self.num_bbox_reg_classes * self.box_dim)
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            "cooling"               : cfg.OWOD.COOLING,
            "weight_known"          : cfg.OWOD.WEIGHT_KNOWN,
            "peak"                  : cfg.OWOD.PEAK,
            "cool_iter"             : cfg.OWOD.COOL_ITER,
            "beta"                  : cfg.OWOD.BETA,
        }
    ...
    def losses(self, predictions, proposals, input_features=None):
        ...
        losses = FastRCNNOutputs(
            ...
            weight_known=self.weight_known,
            peak=self.peak,
            cool_iter=self.cool_iter,
            beta=self.beta,
        ).losses()
...
class CosineSimOutputLayers(FastRCNNOutputLayers):
    """
    Two outputs
    ...
    def __init__(
        ...
        # background class
        # (hence + 1)
        self.cls_score = nn.Linear(self.input_size, self.num_classes + 1)
        ...
        self.bbox_pred = nn.Linear(self.input_size, self.num_bbox_reg_classes * self.box_dim)
        self.sigmoid = nn.Sigmoid()
        self.new_loss = 0
        self.valid_mask = list(range(cfg.OWOD.PREV_INTRODUCED_CLS, cfg.OWOD.PREV_INTRODUCED_CLS + cfg.OWOD.CUR_INTRODUCED_CLS))
        self.invalid_class_range = list(range(self.seen_classes, self.num_classes-1))
        # self.gcn = GraphConvolution(self.input_size, self.input_size)


        self._norm_fact = 1 / math.sqrt(self.input_size // 4)
    ...
    def forward(self, x, pred_classes=None):
        # import pdb; pdb.set_trace()
        if x.dim() > 2:
            x = torch.flatten(x, start_dim=1)          
