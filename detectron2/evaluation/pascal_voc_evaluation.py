...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg=None):
        ...
        self._logits = 0
        self._logits_num = 0
        self.top_unknown_num = 0
        self.top2_unknown_num = 0
        self._scores = 0
        if cfg is not None:
    ...
    def process(self, inputs, outputs, out0_instances=None, out1_instances=None, out2_instances=None, num=0, avg=False):
        for input, output in zip(inputs, outputs):
            image_id = input["image_id"]
            instances = output["instances"].to(self._cpu_device)
            #instances = Instances.cat([instances, out0_instances, out1_instances, out2_instances])
            boxes = instances.pred_boxes.tensor.numpy()
            scores = instances.scores.tolist()
            classes = instances.pred_classes.tolist()
            logits = instances.logits
            self._logits_num += logits.size(0)
            self._logits += torch.sum(logits, dim=0)
            topk_values, indices = torch.topk(logits, 2, dim=1)
            self.top_unknown_num += sum(indices[:, 0] == 80)
            self.top2_unknown_num += sum(indices[:, 1] == 80)
            self._scores += torch.sum(torch.softmax(logits, dim=1), dim=0)
            classes = self.update_label_based_on_energy(logits, classes)
            #tmp = pairwise_iou(instances.pred_boxes, instances.pred_boxes)
            #mask_ind_tmp = (tmp > 0.8).nonzero().tolist()
            #mask_ind = [x for x in mask_ind_tmp if x[0] < x[1]]
            #if len(mask_ind) > 0:
            #    num = num + 1
            if avg:
                tmp = pairwise_iou(instances.pred_boxes, instances.pred_boxes)
                mask_ind_tmp = (tmp > 0.7).nonzero().tolist()
                mask_ind = [x for x in mask_ind_tmp if x[0] < x[1]]
                d = {}
                del_list = set()
                for first, second in mask_ind:
                    del_list.add(second)
                    if first in del_list:
                        continue
                    if first not in d:
                        d[first] = {}
                        d[first][classes[first]] = [scores[first], 1]
                    if classes[second] not in d[first]:
                        d[first][classes[second]] = [scores[second], 1]
                    else:
                        d[first][classes[second]][0] = d[first][classes[second]][0] + scores[second]
                        d[first][classes[second]][1] = d[first][classes[second]][1] + 1
                    
                for k, clas in d.items():
                    max_clss = ''
                    max_score = -1
                    for clss, s_n in clas.items():
                        if (s_n[0] / s_n[1]) > max_score:
                            max_score = s_n[0] / s_n[1]
                            max_clss = clss
                    # clss = max(v.items(), key=operator.itemgetter(1))[0]
                    d[k]['cls'] = max_clss
                    d[k]['score'] = max_score
                boxes = boxes.tolist()
                new_boxes = []
                ...
                for i in range(len(instances)):
                    ...
                    if i in d:
                        new_classes.append(d[i]['cls'])
                        new_scores.append(d[i]['score'])
                    else:
            ...
            #for box, score, cls in zip(new_boxes, new_scores, new_classes):
                if cls == -100:
                    continue
                ...
                # if cls in range(81, 85):
                #    self._predictions[80].append(
                #        f"{image_id} {score:.3f} {xmin:.1f} {ymin:.1f} {xmax:.1f} {ymax:.1f}"
                #    ) 
    ...
    def evaluate(self):
        ...
        for key, v in B.items():
            if v['num'] !=0:
                ECE += v['num'] * (abs(v['acc'] / v['num'] - v['conf'] / v['num']))
                OE += v['num'] * (v['conf'] / v['num'] * max(v['conf'] / v['num'] - v['acc'] / v['num'], 0))
        ECE /= all_num
        OE /= all_num
        ...
        total_num_unk = num_unks[50][0]
        self._logger.info("ECE: {}".format(ECE*100))
        self._logger.info("OE: {}".format(OE*100))
        self._logger.info("top2 unknown: {}".format(self.top2_unknown_num / self._logits_num*100))
        self._logger.info("top unknown: {}".format(self.top_unknown_num / self._logits_num*100))
        self._logger.info("logits: " + str(['%.5f' % x for x in (self._logits / self._logits_num).tolist()]))
        self._logger.info("scores: " + str(['%.5f' % x for x in (self._scores / self._logits_num).tolist()]))
        self._logger.info("unknown num:{}".format(total_num_unk))
        self._logger.info("unknown recall num:{}".format(unknown_is_detected))
        self._logger.info("unknown right num:{}".format( unknown_pred_correct))
        if total_num_unk != 0:
            self._logger.info("detected percent:{}".format(float(unknown_is_detected)/total_num_unk*100))
        if unknown_is_detected!=0:
            self._logger.info("correct percent:{}".format(float(unknown_pred_correct)/unknown_is_detected*100))
...
def voc_eval(B, detpath, annopath, imagesetfile, classname, ovthresh=0.5, use_07_metric=False, known_classes=None):
    ...
    for d in range(nd):
        ...
        import math
        bin = math.floor(confidence[d] * 10) / 10
        B['n'] += 1
        B[bin]['num'] += 1
        B[bin]['conf'] += confidence[d]
