...
class OWEvaluator:
    ...
    def update(self, predictions):
        for img_id, pred in predictions.items():
            pred_boxes, pred_labels, pred_scores, logits = [pred[k].cpu() for k in ['boxes', 'labels', 'scores', 'logits']]
    ...
    def accumulate(self):
        for class_label_ind, class_label in enumerate(self.voc_gt.CLASS_NAMES):
            ...
            self.rec, self.prec, self.AP[class_label_ind, ovthresh_ind], self.unk_det_as_known, \
                self.num_unk, self.tp_plus_fp_closed_set, self.fp_open_set = voc_eval(self.B, lines_by_class, \
    ...
    def summarize(self, fmt='{:.06f}'):
        ECE = 0
        OE = 0
        all_num = self.B.pop('n')
        for key, v in self.B.items():
            if v['num'] !=0:
        ...
        print("OE: {}".format(OE*100))
        print("top2 unknown: {}".format(self.top2_unknown_num / self._logits_num * 100))
        print("top unknown: {}".format(self.top_unknown_num / self._logits_num * 100))
        print("logits: " + str(['%.5f' % x for x in (self._logits / self._logits_num).tolist()]))
        print("scores: " + str(['%.5f' % x for x in (self._scores / self._logits_num).tolist()]))
        print("unknown num:{}".format(total_num_unk))
        print("unknown recall num:{}".format(unknown_is_detected))
        print("unknown right num:{}".format( unknown_pred_correct))
        if total_num_unk != 0:
            print("detected percent:{}".format(float(unknown_is_detected)/total_num_unk*100))
        if unknown_is_detected!=0:
            print("correct percent:{}".format(float(unknown_pred_correct)/unknown_is_detected*100))
