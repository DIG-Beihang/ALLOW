...
# 这个是ow-detr 的80类顺序
# ('aeroplane', 'bicycle', 'bird', 'boat', 'bus', 'car', 'cat', 'cow', 'dog', 'horse', 'motorbike', 'sheep',
# 'train', 'elephant', 'bear', 'zebra', 'giraffe', 'truck', 'person', 'traffic light', 'fire hydrant', 'stop sign',
# 'parking meter', 'bench', 'chair', 'diningtable', 'pottedplant', 'backpack', 'umbrella', 'handbag', 'tie',
# 'suitcase', 'microwave', 'oven', 'toaster', 'sink', 'refrigerator', 'bed', 'toilet', 'sofa', 'frisbee',
# 'skis', 'snowboard', 'sports ball', 'kite', 'baseball bat', 'baseball glove', 'skateboard', 'surfboard',
# 'tennis racket', 'banana', 'apple', 'sandwich', 'orange', 'broccoli', 'carrot', 'hot dog', 'pizza', 'donut',
# 'cake', 'laptop', 'mouse', 'remote', 'keyboard', 'cell phone', 'book', 'clock', 'vase', 'scissors', 'teddy bear',
# 'hair drier', 'toothbrush', 'wine glass', 'cup', 'fork', 'knife', 'spoon', 'bowl', 'tvmonitor', 'bottle', 'unknown')
...
class OWDetection(VisionDataset):
    ...
    def __init__(self,
        ...
        for year, image_set in zip(years, image_sets):
            ...
            if year == "2007" and image_set == "test":
                year = "2007-test"
            valid_sets = ["t1_train", "t2_train", "t3_train", "t4_train", "t2_ft", "t3_ft", "t4_ft", "test",
                          "all_task_test"]
            if year == "2007-test":
            ...
            file_names = self.remove_no_target_file(file_names, image_dir, annotation_dir)
    ...
    def remove_no_target_file(self, file_names, image_dir, annotation_dir):
        image_set = self.transforms[0]

        res = []
        removed_count = 0

        def get_instances(img_id, ann_file):
        ...
        for image_path, imgid, ann_file, origin in ((os.path.join(image_dir, x + ".jpg"),
                                                     self.convert_image_id(x, to_integer=True),
                                                     os.path.join(annotation_dir, x + ".xml"),
                                                     x) for x in file_names):
            target, instances = get_instances(imgid, ann_file)
        ...
        print(f"remove no target file: {removed_count}")
